...
serve(async (req) => {
  ...
  try {
    ...
    const data = await fetchFromClashAPI(`/clans/${encodeURIComponent(CLAN_TAG)}/currentwar`, apiToken)
    ...
    console.error('Clan War API Error:', error)
    return new Response(
      JSON.stringify({ 
        error: error.message || 'Failed to fetch clan war data',
